...
function IGW_Sync:Initialize()
    ...
    this:SetScript("OnEvent", IGW_Sync.OnEvent)
...
-- Event handler
function IGW_Sync.OnEvent()
    if event == "CHAT_MSG_ADDON" then
        IGW_Sync:OnAddonMessage(arg1, arg2, arg3, arg4)
...
function IGW_Sync:SendDataRequest(peerName, dataType)
    ...
    requestFrame:SetScript("OnUpdate", function()
        this.timer = this.timer - arg1
        if this.timer <= 0 then
            if this.dataType == "schedule" then
                IGW_Sync:DebugPrint("Requesting schedule from " .. this.peer)
                IGW_Sync:SendMessageTo(IGW_Sync.MSG_TYPE.REQUEST_SCHEDULE, nil, this.peer)
            elseif this.dataType == "announcements" then
                IGW_Sync:DebugPrint("Requesting announcements from " .. this.peer)
                IGW_Sync:SendMessageTo(IGW_Sync.MSG_TYPE.REQUEST_ANNOUNCEMENTS, nil, this.peer)
            end
...
function IGW_Sync:ScheduleHandshake()
    ...
    self.handshakeFrame:SetScript("OnUpdate", function()
        IGW_Sync.handshakeTimer = IGW_Sync.handshakeTimer - arg1
...
function IGW_Sync:HandleDataRequest(sender)
    ...
    responseFrame:SetScript("OnUpdate", function()
        this.timer = this.timer + arg1
        
        if this.timer >= delay then
            -- Check if this request was already answered by someone else
            if IGW_Sync.pendingSyncResponses[this.requestId] then
                -- Send response
                IGW_Sync:SendMessageTo(IGW_Sync.MSG_TYPE.SYNC_DATA, { ack = "1" }, this.sender)
                
                -- Clean up
                IGW_Sync.pendingSyncResponses[this.requestId] = nil
                this:SetScript("OnUpdate", nil)
            else
    ...
    if not self:IsScheduleEmpty() then
        ...
        responseFrame:SetScript("OnUpdate", function()
            ...
            if this.timer <= 0 then
                ...
                local encoded = IGW_Sync:EncodeRaidSchedule(IGW_Sync.data.raidSchedule)
                local timestamp = IGW_Sync.data.raidScheduleTimestamp or 0
                IGW_Sync:DebugPrint("Sending schedule to " .. this.sender .. " with timestamp " .. timestamp)
    ...
    if self.data.announcements and table.getn(self.data.announcements) > 0 then
        ...
        responseFrame:SetScript("OnUpdate", function()
            ...
            if this.timer <= 0 then
                ...
                if recent then
                    IGW_Sync:DebugPrint("Sending announcement to " .. this.sender)
                    IGW_Sync:SendMessageTo(IGW_Sync.MSG_TYPE.ANNOUNCEMENT, recent, this.sender)
                end
                this:SetScript("OnUpdate", nil)
            end
        end)
...
function IGW_Sync:HandleAllDataRequest(sender)
    ...
    if hasSchedule or hasAnnouncements then
        ...
        responseFrame:SetScript("OnUpdate", function()
            ...
            if this.timer <= 0 then
                -- Send schedule first
                if this.hasSchedule and not this.sentSchedule then
                    local encoded = IGW_Sync:EncodeRaidSchedule(IGW_Sync.data.raidSchedule)
                    IGW_Sync:SendMessageTo(IGW_Sync.MSG_TYPE.RAID_SCHEDULE, {encoded = encoded}, this.sender)
                    this.sentSchedule = true
                    this.timer = 1 -- Wait 1 more second before sending announcement
                    return
                end
                
                -- Then send announcement
                if this.hasAnnouncements then
                    local recent = IGW_Sync.data.announcements[1]
                    if recent then
                        IGW_Sync:SendMessageTo(IGW_Sync.MSG_TYPE.ANNOUNCEMENT, recent, this.sender)
                    end
...
syncFrame:SetScript("OnEvent", function()
    if event == "ADDON_LOADED" and arg1 == "ImprovedGuildWindow" then
        IGW_Sync:Initialize()
    end
end)
